...
export function ProofShowcase({
  ...
  description = "Only customer-approved quotes can be published. Every quote also needs your team's approval.",
  ctaHref,
  ctaLabel,
  compact = false,
}: {
    ...
    <div className="geo-premium-card rounded-3xl p-6">
      ...
      <div className={`mt-5 grid gap-3 ${compact ? "xl:grid-cols-[1.05fr_0.95fr]" : "lg:grid-cols-[0.9fr_1.1fr]"}`}>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <ProofMetric label="Above-threshold replies" value={`${summary.publicReady}`} icon={Star} />
        ...
        <div className="space-y-3">
          {featuredAssets.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-[var(--muted)]/20 p-4 text-sm text-[var(--muted-foreground)]">
              No customer-authorized quote assets yet. Customers can optionally grant permission for their written note to be reviewed as a quote.
            </div>
          ) : (
            featuredAssets.map((asset) => (
              <div key={asset.id} className="geo-proof-card rounded-2xl p-4">
                ...
                <p className="mt-2 text-xs text-[var(--muted-foreground)]">
                  {asset.customer_permission_at ? "Customer permission recorded" : "Customer permission required before public use"}
                </p>
